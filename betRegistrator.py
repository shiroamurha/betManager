...
class betManager():
    ...
    def update_values(self):
        ...
        for bet_num in range(1, len(self.database.get('bets'))+1):
            ...
            self.win_counting[0] += 1 if self.database.get('bets').get(str(bet_num)).get('succession') else 0
            self.win_counting[1] += 1 if not self.database.get('bets').get(str(bet_num)).get('succession') else 0
